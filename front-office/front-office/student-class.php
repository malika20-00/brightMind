		...
		<div class="container">
			<div class="row">
				<div class="col-md-9">
					...
					require_once '../include/autoloadEtudiant.php';
					$etudiant = new Etudiant();
					$i = 0;
					//	foreach ($etudiant->getClassByIdStudent($_SESSION['id']) as $affectation) {
					$crudClass = new CRUDClass();
					$class = $crudClass->getClassById($_GET['id']);
					?>
					<section id="course-details" class="course-details-section" style="padding-top: 20px;">
						<div>
							<div>
								<div>
									<div class="affiliate-market-guide mb65">
										...
										<div class="affiliate-market-accordion">
											<div id="accordion" class="panel-group">
												<?php $crudCours = new CRUDCours();
												$cours = $crudCours->readCoursByIdClass($_GET['id']);
														...
														<div class="panel-title" id="heading<?php echo $etudiant->convert_number_to_words($i); ?>">
															<div class="ac-head">
																...
																<button class="btn btn-link" data-toggle="collapse" data-target="#collapse<?php echo $etudiant->convert_number_to_words($i); ?>" aria-expanded="true" aria-controls="collapse<?php echo $etudiant->convert_number_to_words($i); ?>">
																	<span>0<?php echo $j++; ?></span> <?php echo strtoupper($c['titre']); ?>
																</button>
														...
														<div id="collapse<?php echo $etudiant->convert_number_to_words($i); ?>" class="collapse show" aria-labelledby="heading<?php echo $etudiant->convert_number_to_words($i); ?>" data-parent="#accordion">
															<div class="panel-body">
																<div style="float:left;">
																	<?php $chapitre =  $crudCours->fetchAllChapitreByIdCours($c['id']);
																	$pasChap = 0;
																	if (!$chapitre) {
																		$pasChap = 1;
																	}
																	...
																	?>
																	<a <?php if ($pasChap == 1) {
																			echo 'href="student-course-content.php?idClass=' . $c['idClass'] . '&idCours=' . $c['id'] . '"';
																		} else {
																			echo 'href="student-course.php?idClass=' . $c['idClass'] . '&idCours=' . $c['id'] . '"';
																		} ?> class="content"><strong><?php echo strtolower($c['titre']); ?></strong></a>
																...
																<div>
																	<br>
																	<ol>
																		<?php $chapitre =  $crudCours->readChapterByIdCours($c['id']);
																		foreach ($chapitre as $chap) {

																		?>
																			<li><a href="student-course-details.php?idClass=<?php echo $c['idClass'] ?>&idCours=<?php echo $c['id'] ?>&idChapitre=<?php echo $chap['id'] ?>" ; class=" content text-uppercase"><?php echo $chap['titre']; ?> </a>
