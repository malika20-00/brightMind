...
class CreerProf extends dbConnection {
    private $nom;
    private $prenom;
    private $email;
    private $cin;
    ...
    {
        $sql = "select * from professeur where email='" . $this->email . "'";
    ...
    public function sendEmailToProf()
    {
        if ($this->errors == null) {
    ...
    public function createProf()
    {
        if ($this->errors == null) {
            ...
            $idAdmin = $_SESSION['id'];
            $admin->addProf($this->prenom,$this->nom,$this->cin,$this->telephone,$idAdmin);
        }
    }
